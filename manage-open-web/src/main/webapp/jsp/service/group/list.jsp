...
<div class="container" style="width: 96%; padding-top:10px;">
    ...
    <div class="row">
        <table id="groupList" class="table table-bordered table-hover">
            <thead>
            <tr>
                <th width="10%" data-field="id" data-render="renderid">序号</th>
                <th width="65%" data-field="name">分组名称</th>
                <%--<th width="35%" data-field="seq">显示顺序</th>--%>
                <th width="35%" data-field="id" data-render="renderoptions">操作</th>
            </tr>
            </thead>
        </table>
    </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        var url = context + "/service/service/group/list/" + (pid == 'null' ? '-1' : pid);
        ...
        $("#back").bind("click", function () {
            window.location.href = context + "/jsp/service/group/list.jsp";
        });
    });

    function manageChildren(groupId) {
        window.location.href = context + "/jsp/service/group/list.jsp?pid=" + groupId;
    }
    ...
    function del(groupId) {
        $.dialog({
            ...
            ok: function () {
                $.ajax({
                    url: context + "/service/service/group/delete/" + groupId,
    ...
    function add() {
        $.dialog({
            type: "iframe",
            title: "添加服务分组",
            url: context + '/jsp/service/group/add.jsp?pid=' + (pid == 'null' ? '-1' : pid),
            width: 450,
            height: 250,
            onclose: function () {
                reloadGroupList();
    ...
    function modify(groupId) {
        if (typeof(groupId) != 'undefined') {
            $.dialog({
                type: "iframe",
                title: "编辑服务分组",
                url: context + '/service/service/group/edit/' + groupId,
                width: 420,
                height: 200,
                onclose: function () {
                    reloadGroupList();
