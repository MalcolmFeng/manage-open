...
<head>
	...
	<script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            var url = context + "/service/open/api/listAudit"
        ...
        function pass(openServiceId) {
            $.dialog({
                ...
                ok: function() {
                    $.ajax({
                        url : context + "/service/open/api/pass/" + openServiceId,
                        success: function(msg){
                            if(msg == true) {
                                reloadAuditList();
                                sticky("api上线成功！");
                            } else {
                                sticky("api上线失败", 'error', 'center');
                            }
                        }
                    });
                },
        ...
        function reject(openServiceId) {
            $.dialog({
                ...
                ok: function() {
                    $.ajax({
                        url : context + "/service/open/api/reject/" + openServiceId,
                        success: function(msg){
                            if(msg == true) {
                                reloadAuditList();
                                sticky("驳回成功！");
	...
	<div class="row">
		<table id="myauthList" class="table table-bordered table-hover">
			...
			<tr>
				...
				<th width="17%" data-field="description">服务描述</th>
				<th width="13%" data-field="createTime">创建时间</th>
				<th width="13%" data-field="id" data-render="renderOptions">操作</th>
			</tr>
			</thead>
		</table>
	</div>
