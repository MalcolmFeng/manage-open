		...
		<form class="form-horizontal" id="saveForm" name="saveForm" onsubmit="return false;">
			...
			<div id="step_1" class="_step" style="display: none;">
				...
				<div class="form-group">
					...
					<div class="col-xs-10 col-md-10 text-left radio" style="margin-top: 5px;">
						<label><input type="radio" name="scProtocol" value="http" onclick="chooseProtocol()" datatype="verifyProtocol" nullmsg="必填" <c:if test="${serviceDef.scProtocol eq 'http' }"> checked="checked"</c:if>/>http</label>
						<label><input type="radio" name="scProtocol" value="webService" onclick="chooseProtocol()" datatype="verifyProtocol" nullmsg="必填" <c:if test="${serviceDef.scProtocol eq 'webService' }"> checked="checked"</c:if>/>webService</label>
						<span class="Validform_checktip"></span>
					</div>
				</div>
				<div class="form-group" id="scFrameDiv" hidden="true">
					<div class="col-xs-2 col-md-2 control-label">
				...
				<div class="form-group" id="sc_ws_namespaceDiv" hidden="true">
					<div class="col-xs-2 col-md-2 control-label">
						<label class="control-label">命名空间<span class="required">*</span></label>
					</div>
					<div class="col-xs-10 col-md-10 text-left radio" style="margin-top: 5px;">
				...
				<div class="form-group" id="sc_ws_functionDiv" hidden="true">
					<div class="col-xs-2 col-md-2 control-label">
						<label class="control-label">请求方法名<span class="required">*</span></label>
					</div>
					<div class="col-xs-10 col-md-10 text-left radio" style="margin-top: 5px;">
		                   ...
		                   <tr>
				              ...
				              <td>
								  <select name="inputRequired">
									  <option value="1" <c:if test="${item.required eq '1' }">selected="selected"  </c:if> >是</option>
				...
				<div class="form-group">
					...
					<div class="col-xs-10 col-md-10">
						<select id="responseContentType" name="responseContentType" style="width: 40%" class="form-control ue-form Validform_input" datatype="s" nullmsg="请选择返回ContentType">
							<option  value="json" <c:if test="${serviceDef.contentType eq 'json' }">selected="selected"  </c:if> >JSON (application/json;charset=utf-8)</option>
...
//            var version = $("#version").val();
        ...
        function verifyFrame(gets, obj, curform, datatye){
        	...
        	if(!scFrame){
                obj.attr("errormsg", "请选择框架");
                return false;
        	}
        }

        function verifyScwsNameSpace(gets, obj, curform, datatye){
            var sc_ws_namespace = $("#sc_ws_namespace").val();
            if(!sc_ws_namespace){
                obj.attr("errormsg", "请填写命名空间");
                return false;
            }
        ...
        function verifyScwsfunction(gets, obj, curform, datatye){
            var sc_ws_function = $("#sc_ws_function").val();
            if(!sc_ws_function){
                obj.attr("errormsg", "请填写请求方法名");
                return false;
            }
            if(sc_ws_function.indexOf('，') != -1){
                obj.attr("errormsg", "请以英文逗号分隔");
                return false;
            }
            return true
        }
